...
from flask import Flask, render_template, request, url_for, flash, redirect, send_file, render_template,jsonify
import os
import pathlib
import matplotlib.pyplot as plt
import json
...
mannual_df = pd.read_csv('/Users/sharmanikhi/Walfly_main_project/data_csv/sort_40resume.csv')
code_df = pd.read_csv('/Users/sharmanikhi/Walfly_main_project/data_csv/Mannual_Resume40 (6).csv')
...
app = Flask(__name__,  static_url_path='/Users/sharmanikhi/Walfly_main_project/static/images/')
# F:\Walfly_main_project\templates\static\images
#/home/anush/Desktop/flask_name_copy/templates/static/images/
@app.route('/')
def index():
...
def candidate_recomm():
    ...
    if request.method == "POST":
        ...
        if first_name and last_name:
            ...
            print(resume_df40)
            candidate = cr(resume_df40,job_data,3)
...
def upload_csv():
    df = pd.read_csv(os.path.join(base_dir,'data_csv', 'sort_40resume.csv'))
...
def get_chatbot_response():
    ...
    if "job" in user_input:
        recommend_job = 1
        response= 'how many jobs do you want me to recommend'
        return jsonify(response)
    ...
    if  recommend_job == 1:
        ...
        return jsonify(response, recommend_job)


    if "candidate" in user_input:
        recommend_candidate = 1
        recommend_job = 0
        response = 'How many candidate do you want me to recommend'
        return jsonify(response)
    ...
    if recommend_job == 2:
        ...
        recommend_job = 0
        rec = 3
        return jsonify(response,rec)  

    if recommend_candidate == 2:
        response = cr(resume_df40,user_input,user_input_pref2)
